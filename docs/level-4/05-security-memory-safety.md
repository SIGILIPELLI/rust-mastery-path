...
Debug builds insert an explicit overflow check (compare-and-branch-to-panic)
after every arithmetic operation on integers, generated by the compiler as
part of lowering `+`/`-`/`*` to MIR when `debug-assertions` is on; release
builds omit that check by default and let the operation silently wrap
(two's-complement wraparound) purely for performance, since a branch after
every arithmetic op would slow down numeric-heavy code measurably. This is
why the two build profiles can behave differently on the exact same
overflow: it isn't undefined behavior in either case (wrapping in release is
fully defined, just probably not what you meant), it's a deliberate,
documented difference in which profile pays for the check.

"Use-after-free and double-free are prevented by ownership, not something to
defend against" is worth taking literally: those bug classes require a
value to be freed while a reference to it still exists, or freed twice by
two owners racing to drop it — and both are exactly the invariants the
borrow checker's lifetime analysis (Level 2) and the single-owner move
semantics (Level 1) are built to make unrepresentable in safe code. This is
the real dividing line the module is drawing: the borrow checker eliminates
whole bug *classes* statically, while overflow, panics-on-bad-input, and
secret-zeroing are runtime *values and data-handling* concerns the type
system can't see into — no amount of borrow-checking tells the compiler
that a `String` holds a password that shouldn't survive in freed memory.
